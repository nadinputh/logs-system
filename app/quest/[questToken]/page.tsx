...
export default async function QuestPage({ params }: { params: { questToken: string } }) {
  const data = await getQuestData(params.questToken)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-violet-50/20 flex items-start justify-center p-4 pt-8 pb-16">
      <div className="w-full max-w-sm space-y-3">
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-full text-indigo-600 bg-indigo-50">
        ...
        <div className="bg-white rounded-2xl border border-border/60 shadow-sm p-5 space-y-3">
          ...
          <Link
            href="/scan"
            className="flex items-center justify-center gap-2 w-full gradient-primary text-white text-sm font-semibold py-2.5 rounded-xl hover:opacity-90 active:scale-[0.98] transition-all shadow-sm shadow-indigo-200"
          >
